...
# max_concurrency info acquired at:
#https://stackoverflow.com/questions/54853085/discord-py-rewrite-allow-only-one-instance-of-a-command
@bot.command()
...
async def poll(ctx, question, *args):
    ...
    my_poll = discord.Embed(title = f"Poll: {question}", description = description)
    ...
    while True:
        ...
        if isrunning == False:
            ...
            channel = bot.get_channel(838889125771608074)
            await channel.send(embed = results_message)
            poll_running = False
            break
        #await asyncio.sleep(1)
 
@poll.error
async def poll_handler(ctx, error):
    if isinstance(error, commands.MaxConcurrencyReached):
        await ctx.send("There is already an active poll.")

@lfg.error
async def lfg_handler(ctx, error):
    if isinstance(error, commands.MaxConcurrencyReached):
        await ctx.send("There is already an instance of LFG running.")      
